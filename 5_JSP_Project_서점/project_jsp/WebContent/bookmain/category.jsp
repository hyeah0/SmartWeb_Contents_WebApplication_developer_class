                    ...
                    <section class="center_section center_cont_search_pick">
                        ...
                        <div class="book_pick">
                           	...
                           	<c:if test="${search == 1 }">
                           		...
                           		<button class="btn book_sales" onclick="location.href='<%=request.getContextPath() %>/main_getBookCategory.do?${uri_listType }&requestType=book_sales&${uri_nowCgy }&${uri_page }&${uri_search }'" >누적 판매순</button>
                            	<p>|</p>
                            	<button class="btn book_price" onclick="location.href='<%=request.getContextPath()%>/main_getBookCategory.do?${uri_listType }&requestType=book_price&${uri_nowCgy }&${uri_page }&${uri_search }'">낮은 가격순</button>
                            	<p>|</p>
                            	<button class="btn book_recent"  onclick="location.href='<%=request.getContextPath()%>/main_getBookCategory.do?${uri_listType }&requestType=book_recent&${uri_nowCgy }&${uri_page }&${uri_search }'">최신 등록순</button>
                           	
                           	</c:if> 
                           	
                           	<!-- 검색하지 않은 상태 -->
                           	<c:if test="${search != 1 }">
                           		<button class="btn book_sales" onclick="location.href='<%=request.getContextPath() %>/main_getBookCategory.do?${uri_listType }&requestType=book_sales&${uri_nowCgy }&${uri_page }'" >누적 판매순</button>
                            	<p>|</p>
                            	<button class="btn book_price" onclick="location.href='<%=request.getContextPath()%>/main_getBookCategory.do?${uri_listType }&requestType=book_price&${uri_nowCgy }&${uri_page }'">낮은 가격순</button>
                            	<p>|</p>
                            	<button class="btn book_recent"  onclick="location.href='<%=request.getContextPath()%>/main_getBookCategory.do?${uri_listType }&requestType=book_recent&${uri_nowCgy }&${uri_page }'">최신 등록순</button>
                           	</c:if>
                        </div>
                        ...
                        <div class="book_search">
                            ...
                            <form class="book_search_form" method="post" action="main_getBookCategory.do?${uri_listType }&${uri_requestType }&${uri_nowCgy }">
                                ...
                                <!-- 검색어 입력시 -->
                                <c:if test="${search == 1 }">
                                	<input class="search_input" name="search_input" value="${search_input }">
                                </c:if>
                                <!-- 검색어 입력안했을경우 -->
                                <c:if test="${search != 1 }">
					                                ...
					                                <div class="booklist category_booklist"> 
					                                    <div class="booklist_imgArea">
					                                        <a class="category_img_a" href="<%=request.getContextPath() %>/book_detail.do?book_num=${bookList.getBook_num() }">
					                                            <img src="<%=request.getContextPath() %>/book_cover/${bookList.getBook_img() }" alt="${bookList.getBook_name() }"> 
					                                        </a>
					                                    </div>
					                                    <div class="booklist_cont">
					                                        <h3>${bookList.getBook_name() }</h3>
					                                        <p><fmt:formatNumber value="${bookList.getBook_price() }"/> 원 </p>
					                                        <p>${bookList.getBook_writer() } / ${bookList.getBook_publisher() }</p>
					                                    </div>
					                                    
					                                    <div class="booklist_btn">
					                                        <button class="btn" onclick="location.href='<%=request.getContextPath()%>/pay.do?book_num=${bookList.getBook_num() }&order_qty=1'"><i class="fa-brands fa-shopify"></i></button>
			                                            	<button class="btn" onclick="location.href='<%=request.getContextPath()%>/cart_add.do?book_num=${bookList.getBook_num() }&cart_qty=1'"><i class="fa-solid fa-basket-shopping"></i></button>
					                                    </div>
					                                </div>
				                                </div>
			                                </c:forEach>
                            ...
                            <ul class="page_ul">
                              ...
                              <!-- page back -->
                            	...
                            	<c:if test="${page != 1 }">
		                             ...
		                             <li class="page_li">
		                               <a class="page_a page_first" href="<%=request.getContextPath() %>/main_getBookCategory.do?${uri_listType }&${uri_requestType }&${uri_nowCgy }&page=${page-1}">
		                                 <span>
	                                 			<i class="fa-solid fa-chevron-left"></i>
	                                 	</span>
		                               </a>
