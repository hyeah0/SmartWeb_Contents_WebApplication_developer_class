...
public class BookDAO_paging_recentpriceO {
 ...
 // 멤버변수
    ...
    // 객체 변수
    public static BookDAO_paging_recentpriceO instance;             // 싱글턴 : 2단계
    
    // 생성자
    public BookDAO_paging_recentpriceO() {  }                       // 싱글턴 : 1단계
    
    /**
     * @see .객체 생성 메서드
     */
    public static BookDAO_paging_recentpriceO getInstance() {   // 싱글턴 : 3단계
        
        if(instance == null) {
            instance = new BookDAO_paging_recentpriceO();
        }
    ...
    public int getDBCount(String cgy_num) {
        System.out.println("---------bookdao : getDBCount() 메서드 실행중..");
        int totalRecord = 0;
...
/* ---------- 백업 부분 -----------------------------------------------
 // cgy_num=all : nav에서 도서전체 클릭 >> 카테고리 구분없이 1년 누적 판매량 기준
// 이슈 : 출시 후 1년 기준이 필요함..
if(cgy_num.equals("all")) { 
    sql = "select rownum"
        + "     , b.*"
        ...
        + "             where sysdate - order_date <= 365"
        + "             group by book_num"
        + "             order by rownum, order_year desc) o"
        + "     on b.book_num = o.book_num"
        + "  where rownum>= "+ startNo  + "and rownum<=" +endNo
        + "  order by rownum, order_year desc";
    System.out.println("카테고리 도서 전체 sql 실행 완료");
    
}else { // nav에서 카테고리 클릭 >> 카테고리별 1년 누적 판매량 기준
    sql = "select rownum"
        + "     , b.*"
        + "     , case when o.order_year is null then 0 else o.order_year end as order_year"
        + "  from book b"
        ...
        + "    and = rownum>= "+ startNo  + "and rownum<=" +endNo
        + "  order by order_year desc";
    System.out.println("카테고리 선택 sql 실행 완료");
}
   ...
   *  openConn();
            ...
            if(requestSearch.equals("book_sales")) { // 1년 누적 판매량(판매 x 책 확인 불가)
                sql = "select b.*"
                    + "  from book b"
                    + "  join (select book_num "
                    + "              , sum(order_qty) as order_year"
